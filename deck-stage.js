...
(() => {
  ...
  const stylesheet = `
    ...
    .overlay {
      ...
      opacity: 0;
      pointer-events: none;
      transition: opacity 130ms ease, transform 130ms cubic-bezier(.2,.8,.2,1), filter 130ms ease;
...
/* --- Deck-wide "signal lost" transition glitch (Digital Smoke Signals) ---
 * Canvas TV-static + ⚠ flash on every slide change. Lives here (not in the
 * DC logic class) so the design stays a static template and inline text
 * editing remains enabled. Skips the Fact->"How I Found LoRa" pivot, which
 * has its own longer tune-in effect on the slide itself. */
(function () {
  if (window.__smokeGlitch) return;
  window.__smokeGlitch = true;

  const ov = document.createElement('div');
  ov.setAttribute('data-glitch-overlay', '');
  ov.setAttribute('data-omelette-chrome', '');
  ov.style.cssText = 'position:fixed;inset:0;z-index:2147483600;pointer-events:none;opacity:0;background:#000;';
  const cv = document.createElement('canvas');
  cv.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;';
  const label = document.createElement('div');
  label.textContent = '\u26A0';
  label.style.cssText = "position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);font-family:'Space Mono',monospace;font-size:4vw;color:#9DE84B;text-shadow:0 0 18px rgba(157,232,75,.8);white-space:nowrap;";
  ov.appendChild(cv);
  ov.appendChild(label);

  const attach = () => { if (document.body && !ov.isConnected) document.body.appendChild(ov); };
  if (document.body) attach(); else document.addEventListener('DOMContentLoaded', attach);

  const ctx = cv.getContext('2d');
  let raf = null, endAt = 0;

  const drawNoise = () => {
    const w = cv.width = Math.floor(innerWidth / 3);
    const h = cv.height = Math.floor(innerHeight / 3);
    const img = ctx.createImageData(w, h);
    const d = img.data;
    for (let i = 0; i < d.length; i += 4) {
      const v = (Math.random() * 110) | 0;
      d[i] = 14 + v * 0.55; d[i + 1] = 10 + v * 0.42; d[i + 2] = 24 + v * 0.85; d[i + 3] = 255;
    }
    ctx.putImageData(img, 0, 0);
    for (let b = 0; b < 4; b++) {
      const y = (Math.random() * h) | 0;
      const bh = 2 + ((Math.random() * 6) | 0);
      const dx = ((Math.random() - 0.5) * w * 0.3) | 0;
      ctx.drawImage(cv, 0, y, w, bh, dx, y, w, bh);
    }
  };

  const tick = () => {
    if (endAt - performance.now() <= 0) {
      ov.style.transition = 'opacity 160ms ease';
      ov.style.opacity = '0';
      raf = null;
      return;
    }
    label.style.opacity = Math.random() > 0.25 ? '1' : '0.2';
    drawNoise();
    raf = requestAnimationFrame(tick);
  };

  document.addEventListener('slidechange', (e) => {
    if (!e.detail || e.detail.reason === 'init') return;
    if (matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    // Fact -> "How I Found LoRa" has its own tune-in effect on the slide.
    const slides = e.detail.slide && e.detail.slide.parentElement
      ? Array.from(e.detail.slide.parentElement.children) : null;
    if (e.detail.slide && e.detail.slide.getAttribute('data-label') === 'How I Found LoRa') return;
    attach();
    endAt = performance.now() + 260;
